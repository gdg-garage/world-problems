...
["world_problems", "package:world_problems/world_problems.dart", , U, {
  "^": "",
  WorldProblem: {
    "^": "Object;id>,text,author,source,url,submitter",
    static: {WorldProblem$fromMap: function(data) {
        var t1 = J.getInterceptor$asx(data);
        return new U.WorldProblem(t1.$index(data, "id"), t1.$index(data, "text"), t1.$index(data, "author"), t1.$index(data, "source"), t1.$index(data, "url"), t1.$index(data, "submitter"));
      }}
  },
  WorldProblemsPair: {
    "^": "Object;firstProblem<,thirdProblem"
  },
  Fetcher: {
    "^": "Object;apiUrl",
    fetchRandom$2: function(firstIgnore, thirdIgnore) {
      var completer, t1, url;
      completer = H.setRuntimeTypeInfo(new P._AsyncCompleter(P._Future$(null)), [null]);
      if (firstIgnore.get$length(firstIgnore) > 0)
        t1 = thirdIgnore.get$length(thirdIgnore) > 0;
      else
        t1 = false;
      if (t1) {
        if (firstIgnore.get$length(firstIgnore) > 1 || thirdIgnore.get$length(thirdIgnore) > 1)
          throw H.wrapException(P.Exception_Exception("Longer ignore list than 1 not implemented."));
        url = this.apiUrl + "?ignore_first=" + H.S(J.get$id$x(firstIgnore.get$single(firstIgnore))) + "&ignore_third=" + H.S(J.get$id$x(thirdIgnore.get$single(thirdIgnore)));
      } else
        url = this.apiUrl;
      W.HttpRequest_getString(url, null, null).then$1(new U.Fetcher_fetchRandom_closure(this, completer));
      return completer.future;
    }
  },
  Fetcher_fetchRandom_closure: {
    "^": "Closure:10;this_0,completer_1",
    call$1: function(json) {
      var t1, t2, firstProblem, thirdProblem;
      t1 = J.$index$asx(C.JsonCodec_null_null.decode$1(json), "items");
      t2 = J.getInterceptor$asx(t1);
      firstProblem = U.WorldProblem$fromMap(t2.$index(t1, 0));
      thirdProblem = U.WorldProblem$fromMap(t2.$index(t1, 1));
      t1 = this.completer_1.future;
      if (t1._state !== 0)
        H.throwExpression(P.StateError$("Future already completed"));
      t1._asyncComplete$1(new U.WorldProblemsPair(firstProblem, thirdProblem));
    }
  },
  PageView: {
  ...
  WorldProblemsApp: {
    "^": "Object;fetcher,page,isInitialized,firstProblemsHistory,thirdProblemsHistory,maxHistory",
    initialize$3: function(buttonId, firstId, thirdId) {
      var reload, first, second, t1;
      reload = document.querySelector(buttonId);
      first = document.querySelector(firstId);
      second = document.querySelector(thirdId);
      this.page = new U.PageView(reload, U.ProblemView$(first), U.ProblemView$(second));
      t1 = J.get$onClick$x(reload);
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new U.WorldProblemsApp_initialize_closure(this)), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    ...
    getRandomPair$0: function() {
      return this.fetcher.fetchRandom$2(this.firstProblemsHistory, this.thirdProblemsHistory).then$1(new U.WorldProblemsApp_getRandomPair_closure(this));
    },
    WorldProblemsApp$2$maxHistory: function(fetcher, maxHistory) {
      var t1;
      this.isInitialized = false;
      t1 = H.setRuntimeTypeInfo(new P.ListQueue(null, 0, 0, 0), [null]);
      t1.ListQueue$1(null, null);
      this.firstProblemsHistory = t1;
      t1 = H.setRuntimeTypeInfo(new P.ListQueue(null, 0, 0, 0), [null]);
      t1.ListQueue$1(null, null);
  ...
  WorldProblemsApp_getRandomPair_closure: {
    ...
    call$1: function(pair) {
      var t1, t2, t3;
      t1 = this.this_0;
      t1.firstProblemsHistory._add$1(pair.get$firstProblem());
      t2 = t1.firstProblemsHistory;
      t3 = t1.maxHistory;
      if (t2.get$length(t2) > t3)
        t1.firstProblemsHistory.removeFirst$0();
      t1.thirdProblemsHistory._add$1(pair.thirdProblem);
      t2 = t1.thirdProblemsHistory;
      if (t2.get$length(t2) > t3)
        t1.thirdProblemsHistory.removeFirst$0();
      return pair;
    }
  }
}],
["", "world_problems.dart", , N, {
  "^": "",
  main: [function() {
    var fetcher, app;
    fetcher = new U.Fetcher("/_ah/api/problems/v1/random");
    app = new U.WorldProblemsApp(fetcher, null, null, null, null, 1);
...
function dart_precompiled($collectedClasses) {
  ...
  function WorldProblem(id, text, author, source, url, submitter) {
    this.id = id;
    this.text = text;
    this.author = author;
    this.source = source;
  ...
  WorldProblem.builtin$cls = "WorldProblem";
  if (!"name" in WorldProblem)
    WorldProblem.name = "WorldProblem";
  $desc = $collectedClasses.WorldProblem;
  if ($desc instanceof Array)
    $desc = $desc[1];
  WorldProblem.prototype = $desc;
  WorldProblem.prototype.get$id = function(receiver) {
    return this.id;
  };
  function WorldProblemsPair(firstProblem, thirdProblem) {
    this.firstProblem = firstProblem;
    this.thirdProblem = thirdProblem;
  }
  WorldProblemsPair.builtin$cls = "WorldProblemsPair";
  if (!"name" in WorldProblemsPair)
    WorldProblemsPair.name = "WorldProblemsPair";
  $desc = $collectedClasses.WorldProblemsPair;
  if ($desc instanceof Array)
    $desc = $desc[1];
  WorldProblemsPair.prototype = $desc;
  WorldProblemsPair.prototype.get$firstProblem = function() {
  ...
  function Fetcher_fetchRandom_closure(this_0, completer_1) {
    this.this_0 = this_0;
    this.completer_1 = completer_1;
  }
  Fetcher_fetchRandom_closure.builtin$cls = "Fetcher_fetchRandom_closure";
  if (!"name" in Fetcher_fetchRandom_closure)
    Fetcher_fetchRandom_closure.name = "Fetcher_fetchRandom_closure";
  $desc = $collectedClasses.Fetcher_fetchRandom_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Fetcher_fetchRandom_closure.prototype = $desc;
  function PageView(reload, first, third) {
    this.reload = reload;
    this.first = first;
    this.third = third;
  }
  PageView.builtin$cls = "PageView";
  if (!"name" in PageView)
    PageView.name = "PageView";
  $desc = $collectedClasses.PageView;
  if ($desc instanceof Array)
    $desc = $desc[1];
